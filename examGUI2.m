...
for i = 1:1:2
    ...
    if i == 1
        polyH = roipoly;
        IfaseVI=((double(Ifase).*double(polyH))+pi)*180/pi;
        Iz=find(IfaseVI ~=180);
        FaseI=IfaseVI(Iz);
        meanOfPolyH= mean(FaseI(:));
        set(handles.meanL, 'String', num2str(mean(meanOfPolyH)));
        set(handles.modeL, 'String', num2str(mode(FaseI)));
        assignin('base', 'caca', FaseI);
        set(handles.stdL, 'String', num2str(std(FaseI)));
        assignin('base', 'meanH', meanOfPolyH);
    else
        polyR = roipoly;
        IfaseVD=((double(Ifase).*double(polyR))+pi)*180/pi;
        Id=find(IfaseVD~=180);
        FaseD=IfaseVD(Id);
        meanOfPolyR = mean(FaseD);
        set(handles.meanR, 'String', num2str((meanOfPolyR)));
        set(handles.modeR, 'String', num2str(mode(FaseD)));
        set(handles.stdR, 'String', num2str(std(FaseD)));
